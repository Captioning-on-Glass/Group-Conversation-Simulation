...
parse_arguments(int argc, char *argv[]) {
    ...
    while (cmd_opt) {
        ...
        switch (cmd_opt) {
            ...
            case 'p':
                ...
                break;
            case 's':
                fov = (int) std::stoi(optarg);
                num_chars = fov_angle_to_num_chars(fov);
                box_pixel_width = fov_angle_to_box_pixel_width(fov);
                printf("FOV is %i\n", fov);
            case '?':
            default:
    ...
    return std::make_tuple(video_section, presentation_method, half_fov, foreground_color, background_color, path_to_font, fov, num_chars, box_pixel_width);
}
